 ...
 */
template<typename K, size_t Error = 64, size_t RecursiveError = 16, size_t CacheSize = 1024, typename Floating = double>
class PGMIndex {
    ...
    // auto exponentialSearch() {


    //     return std::upper_bound(lo, hi, key);
    // }
...
public:
    ...
    PGMIndex(RandomIt first, RandomIt last)
      : n(std::distance(first, last)), first_key(*first), segments(), levels_sizes(), levels_offsets(), cache(RandomCache(CacheSize)) {
     ...
     */
    void add_cache_pos(const uint64_t &key, const uint64_t &pos)
    {
      cache.add_pos(key, pos);
    }
    ...
    ApproxPos find_approximate_position(const K &key) {
      ...
      std::chrono::duration<double, std::milli> elapsedTime = (elapsed - start);
      cacheTime += elapsedTime.count();
    ...
    if(cache_res.second) {
      ...
      lo = SUB_ERR(pos, Error);
      hi = ADD_ERR(pos, Error, n);
	}
	    /*
      auto k = std::max(first_key, key);
      auto it = segment_for_key(k);
...
template<typename K, size_t Error, size_t RecursiveError, size_t CacheSize, typename Floating>
struct PGMIndex<K, Error, RecursiveError, CacheSize, Floating>::Segment {
...
using BinarySearchBasedPGMIndex = PGMIndex<K, Error, 0, 16, Floating>;
