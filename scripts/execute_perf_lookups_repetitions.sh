...
function do_benchmark() {
    ...
    RESULTS=./results/$1_perf_repetition_results_$2.txt
    if [ -f $RESULTS ]; then
        echo "Already have results for $1 ($2)"
    else
        echo "Executing workload $1 ($2)"
        $BENCHMARK -r 1 --perf ./data/$1 ./data/$1_equality_lookups_repetitions_$2 --pareto | tee ./results/$1_perf_repetition_results_$2.txt
    fi
}
